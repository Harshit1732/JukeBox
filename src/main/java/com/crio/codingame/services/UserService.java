...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        ...
        {
            user.sort(Comparator.comparingInt(User:: getScore));
        }else if(scoreOrder==scoreOrder.DESC)
        {
            user.sort(Comparator.comparingInt(User:: getScore).reversed());
        }
    ...
    public UserRegistrationDto withdrawContest(String contestId, String userName) throws ContestNotFoundException, UserNotFoundException, InvalidOperationException {
        ...
        if(!user.checkIfContestExists(contest)){
            throw new InvalidOperationException("Cannot withdearm from Contest. Contest for given id:"+contestId+" is not registered");
        }
        if(contest.getCreator().getName().equals(userName))
        {
            throw new InvalidOperationException("cannot wihdraw from contest.contest is created by this "+userName+"");
        }
