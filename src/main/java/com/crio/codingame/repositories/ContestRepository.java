...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAll() {
     return contestMap.entrySet().stream().map(Map.Entry:: getValue).collect(Collectors.toList());
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
     return contestMap.entrySet().stream().filter(e->e.getValue().getLevel().equals(level)).map(Map.Entry::getValue).collect(Collectors.toList());
    }
